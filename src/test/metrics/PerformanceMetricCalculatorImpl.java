...
public class PerformanceMetricCalculatorImpl implements PerformanceMetricCalculator
{
	...
	public String populateDetectionMetricsAndGetLog(List<Object> alignment, TestoutDTO testoutDTO) 
	{
		...
		if((detectionTruePositive.size() + detectionFalsePositive.size()) != 0)
			detectionPrecision = 100 * detectionTruePositive.size() / (detectionTruePositive.size() + detectionFalsePositive.size());
		
		if((detectionTruePositive.size() + detectionFalseNegative.size()) != 0)
			detectionRecall = 100 * detectionTruePositive.size() / (detectionTruePositive.size() + detectionFalseNegative.size());		
		
		
		testoutDTO.detectionPrecision = detectionPrecision;
		testoutDTO.detectionRecall = detectionRecall;		
	...
	{
		float correctionPrecision = -1;
		float correctionRecall = -1;
		...
		if((correctionTruePositive.size() + correctionFalsePositive.size()) != 0)
			correctionPrecision = 100 * correctionTruePositive.size() / (correctionTruePositive.size() + correctionFalsePositive.size());
		
		if((correctionTruePositive.size() + correctionFalseNegative.size()) != 0)
			correctionRecall = 100 * correctionTruePositive.size() / (correctionTruePositive.size() + correctionFalseNegative.size());		
	
		
		testoutDTO.correctionPrecision = correctionPrecision;
		testoutDTO.correctionRecall = correctionRecall;
